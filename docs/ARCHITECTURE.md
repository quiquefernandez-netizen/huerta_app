...
El adaptador `SupabaseDataService` llama por HTTPS a funciones PostgreSQL y añade el token de sesión cuando Auth lo proporcione, sin guardar ese token en la configuración. Agrupar la lectura de Fase 1 en `get_community_snapshot` y añadir consultas RPC específicas para los módulos posteriores permite devolver solo el resumen permitido al usuario y evita dar acceso directo a las tablas. El catálogo documental conserva únicamente metadatos y referencias HTTPS; no existe todavía subida de archivos ni una decisión cerrada sobre su proveedor de almacenamiento.
